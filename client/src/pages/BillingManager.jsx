...
import { Banknote, Search, Users, Trash2, Check, Calculator, FileSpreadsheet, Layers, FolderHeart, AlertTriangle, RefreshCcw } from 'lucide-react';
...
const BillingManager = () => {
    ...
    // States Onglet Calcul / Historique
    const [startDate, setStartDate] = useState(format(startOfMonth(new Date()), 'yyyy-MM-dd'));
    const [endDate, setEndDate] = useState(format(endOfMonth(new Date()), 'yyyy-MM-dd'));
    const [calculatedInvoices, setCalculatedInvoices] = useState([]);
    const [isCalculating, setIsCalculating] = useState(false);
    const [hasExistingData, setHasExistingData] = useState(false); // NOUVEAU: Savoir si on écrase

    useEffect(() => { 
        api.get(`/children`).then(res => setChildren(Array.isArray(res.data) ? res.data : []));
    }, []);

    // NOUVEAU : Quand la date change, on regarde si des factures existent déjà en BDD
    useEffect(() => {
        if (activeTab === 'calc') loadExistingInvoices();
    ...
    const loadExistingInvoices = async () => {
        ...
        } catch (e) { console.error("Erreur chargement historique"); }
    };

    const triggerGeneration = async (forceOverwrite = false) => {
        if (forceOverwrite && !window.confirm("⚠️ ATTENTION : Cela va écraser les factures brouillons existantes pour cette période. Continuer ?")) {
            return;
        }
        ...
        try {
            ...
            alert(forceOverwrite ? "Facturation recalculée et écrasée avec succès !" : "Facturation générée et sauvegardée !");
        } catch (e) {
            if (e.response?.status === 409) {
                alert("⚠️ Une facturation existe DÉJÀ pour cette période. Utilisez le bouton de recalcul si vous souhaitez l'écraser.");
            } else if (e.response?.status === 403) {
                alert("⛔ IMPOSSIBLE : Des factures sont déjà verrouillées/publiées sur cette période.");
            } else {
    ...
    // --- LE RESTE DU CODE (Pdf, Garde alternée) RESTE 100% IDENTIQUE ---
    const filteredSearch = useMemo(() => {
        if (search.length < 2 || !Array.isArray(children)) return [];
        const query = search.toLowerCase();
        return children.filter(c => (c.lastName?.toLowerCase() || "").includes(query) || (c.firstName?.toLowerCase() || "").includes(query));
    }, [children, search]);

    const selectChild = async (child) => { setSelectedChild(child); setSearch(''); loadBillings(child._id); };

    const loadBillings = async (childId) => {
        try { const { data } = await api.get(`/billing/child/${childId}`); setBillings(Array.isArray(data) ? data : []); } catch (e) { setBillings([]); }
    };
    ...
    const handleAddBilling = async (e) => {
        ...
        try {
            ...
            setBillToFamilyId(''); setSelectedDates([]); loadBillings(selectedChild._id);
    ...
    const totalRecettesCommune = useMemo(() => calculatedInvoices.reduce((sum, inv) => sum + inv.totalGlobal, 0).toFixed(2), [calculatedInvoices]);
    ...
    const exportInvoicePDF = (invoice) => {
        ...
        const tableBody = invoice.items.map(item => [ item.label, item.count.toString(), `${item.unitPrice.toFixed(2)} €`, `${item.total.toFixed(2)} €` ]);
    ...
    const exportGeneralRecapPDF = () => {
        ...
        doc.setFont("helvetica", "bold"); doc.setFontSize(16); doc.setTextColor(30, 41, 59); doc.text("VILLE DE CARIGNAN-DE-BORDEAUX", 14, 20);
        doc.setFontSize(11); doc.text(`RÔLE GÉNÉRAL DE RECETTES - PÉRISCOLAIRE & CANTINE`, 14, 28);
        doc.setFont("helvetica", "normal"); doc.setFontSize(9);
        doc.text(`Période de facturation : Du ${startStr} au ${endStr}\nGénéré par Carillon le : ${dateGeneration}`, 14, 34);
        ...
        const tableBody = calculatedInvoices.map((inv) => {
            ...
            return [ inv.reference || '-', inv.payeur, detailStr, `${inv.totalGlobal.toFixed(2)} €` ];
        });

        const grandTotal = calculatedInvoices.reduce((sum, inv) => sum + inv.totalGlobal, 0);
        tableBody.push([ "", "TOTAL GÉNÉRAL DU RÔLE", `${calculatedInvoices.length} familles facturées`, `${grandTotal.toFixed(2)} €` ]);

        autoTable(doc, {
            startY: 45, head: tableHead, body: tableBody, theme: 'grid',
            headStyles: { fillColor: [30, 41, 59], textColor: 255, fontStyle: 'bold' },
            styles: { font: 'helvetica', fontSize: 9, cellPadding: 4, valign: 'middle' },
            columnStyles: { 0: { halign: 'center', width: 25 }, 1: { fontStyle: 'bold', width: 50 }, 2: { fontSize: 8 }, 3: { halign: 'right', fontStyle: 'bold', width: 30 } },
            didParseCell: (data) => {
                if (data.row.index === tableBody.length - 1) {
                    data.cell.styles.fillColor = [241, 245, 249]; data.cell.styles.fontStyle = 'bold';
                    if (data.column.index === 3) data.cell.styles.textColor = [30, 58, 138];
                }
            }
        });
    ...
    const exportAllInvoicesCombinedPDF = () => {
        ...
        calculatedInvoices.forEach((invoice, index) => {
            ...
            doc.setFont("helvetica", "normal"); doc.text(`Page ${index + 1} / ${calculatedInvoices.length}`, 170, doc.internal.pageSize.getHeight() - 10);
        });
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col pb-20">
            ...
            {activeTab === 'calc' && (
                <div className="max-w-6xl mx-auto w-full p-4 md:p-8 space-y-6">
                    <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                        ...
                        {/* BOUTONS ADAPTATIFS SELON LA PRÉSENCE DE DONNÉES */}
                        {!hasExistingData ? (
                        ...
                        <div className="space-y-6 animate-in fade-in duration-300">
                            ...
                            <div className="bg-car-dark text-white p-8 rounded-[2rem] shadow-xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden">
                                ...
                                <div className="flex flex-wrap gap-3 z-10 w-full lg:w-auto">
                                    ...
                                    <button onClick={() => alert("Simulation d'exportation XML PES V2 gélée.")} className="bg-slate-700/50 text-slate-400 cursor-not-allowed px-5 py-3 rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 flex-1 lg:flex-none justify-center border border-slate-600/50">
                                        Export PES_V2
                                    </button>
                                </div>
                            </div>

                            {/* LISTE DES FACTURES (Désormais persistantes) */}
                            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                    <div className="flex items-center gap-3">
                                        <div className="bg-white p-2 rounded-lg shadow-sm border border-slate-200"><Layers className="text-car-blue" size={20}/></div>
                                            ...
                                            <div className="space-y-2 flex-1">
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-2">
                                                    {inv.items.map((item, iIdx) => (
                                                        <span key={iIdx} className="text-[10px] bg-white border border-slate-200 text-slate-600 font-bold px-2 py-1 rounded-md shadow-sm">
            ...
            {/* CONTENU ONGLET 2 : GARDE ALTERNÉE (Inchangé) */}
