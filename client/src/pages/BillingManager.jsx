...
const BillingManager = () => {
    ...
    useEffect(() => { 
        api.get(`/children`)
           ...
           .catch(err => {
               console.error("Erreur chargement enfants (Billing):", err);
               setChildren([]);
           });
    }, []);
    ...
    const handleAddBilling = async (e) => {
        ...
        try {
            ...
            alert("Erreur lors de l'enregistrement de la facturation.");
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
            <div className="bg-white shadow-sm z-20 sticky top-0 p-4 border-b border-slate-100 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    <Banknote className="text-car-blue"/>
                    <h1 className="font-black text-car-dark text-xl">Facturation Alternée</h1>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto w-full p-4 md:p-8 space-y-6">
                <div className="relative">
                    ...
                    <input 
                        type="text" 
                        className="w-full pl-14 p-4 bg-white shadow-sm border border-slate-100 rounded-[2rem] focus:ring-4 focus:ring-car-blue/20 outline-none font-bold text-car-dark placeholder:text-slate-400 transition-all text-lg" 
                    ...
                    <div className="bg-slate-100 rounded-[2rem] p-2 animate-in fade-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <form onSubmit={handleAddBilling} className="bg-white rounded-[1.5rem] p-6 shadow-sm border border-slate-200 flex flex-col">
                                ...
                                <input 
                                    type="text" 
                                    className="w-full bg-slate-50 border-none p-4 rounded-2xl focus:ring-4 focus:ring-car-blue/20 outline-none font-bold text-car-dark mb-4" 
                                    placeholder="Nom à facturer (Ex: Maman, Papa...)" 
                                    value={billTo} 
